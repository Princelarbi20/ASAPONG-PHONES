    ...
    <div className="w-full aspect-[4/3] bg-slate-50/60 rounded-lg relative overflow-hidden mb-2.5">
        ...
        {images && images.length > 0 ? (
          images.map((img, i) => (
            <div key={i} className="h-full flex items-center justify-center p-1.5" style={{ width: `${100 / images.length}%` }}>
              <img src={getSingleImageUrl(img)} alt={`${name} view ${i + 1}`} className="max-w-full max-h-full object-contain pointer-events-none" />
            </div>
          ))
        ) : (
          <div className="w-full h-full flex items-center justify-center p-1.5">
            <img src={getSingleImageUrl(null)} alt={name} className="max-w-full max-h-full object-contain" />
          </div>
        )}
      </div>
      ...
      {isItemSoldOut && (
        <div className="absolute inset-0 bg-white/70 backdrop-blur-xs flex items-center justify-center z-20">
          <span className="text-[9px] font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded-md uppercase tracking-wide border border-slate-200">Sold Out</span>
        </div>
      )}
    </div>
  );
...
const ProductList = ({ category, linkTo }) => {
  ...
  useEffect(() => {
    ...
    const autoScrollInterval = setInterval(() => {
      ...
      const cardWidth = cardElement.getBoundingClientRect().width + 16; // card width + flex gap layout spacing
  ...
  return (
    <div className="w-full bg-slate-50 px-3 sm:px-6 py-8 font-sans overflow-hidden">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-xs text-slate-400 font-medium italic bg-white border border-dashed border-slate-200 rounded-xl">
